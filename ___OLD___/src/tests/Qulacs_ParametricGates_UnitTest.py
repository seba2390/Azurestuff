...
def generate_rxx_test_cases(nr_rng_trials: int, use_native: bool) -> List[Tuple[np.ndarray, np.ndarray]]:
    test_cases = []
    for seed in range(nr_rng_trials):
        ...
        qulacs_circuit = qulacs.ParametricQuantumCircuit(__N__)
        parametric_RXX(circuit=qulacs_circuit, angle=theta, qubit_1=int(pair[0]), qubit_2=int(pair[1]),
...
def generate_ryy_test_cases(nr_rng_trials: int, use_native: bool) -> List[Tuple[np.ndarray, np.ndarray]]:
    test_cases = []
    for seed in range(nr_rng_trials):
        np.random.seed(seed)
        theta = np.random.uniform(-2 * np.pi, 2 * np.pi)
        ...
        qulacs_state_vector = qulacs.QuantumState(__N__)
        qulacs_circuit = qulacs.ParametricQuantumCircuit(__N__)
        parametric_RYY(circuit=qulacs_circuit, angle=theta, qubit_1=int(pair[0]), qubit_2=int(pair[1]),
                       use_native=use_native)
        qulacs_circuit.update_quantum_state(qulacs_state_vector)
        qulacs_state_vector = qulacs_state_vector.get_vector()
...
def generate_rz_test_cases(nr_rng_trials: int = 10) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    for seed in range(nr_rng_trials):
        ...
        qulacs_circuit.add_gate(qulacs.gate.ParametricRZ(index=idx, angle=-theta))
        qulacs_circuit.update_quantum_state(qulacs_state_vector)
        qulacs_state_vector = qulacs_state_vector.get_vector()
...
N_RNG_TRIALS = 10
rxx_test_cases = generate_rxx_test_cases(nr_rng_trials=N_RNG_TRIALS, use_native=False)
rxx_test_cases_2 = generate_rxx_test_cases(nr_rng_trials=N_RNG_TRIALS, use_native=True)
ryy_test_cases = generate_ryy_test_cases(nr_rng_trials=N_RNG_TRIALS, use_native=False)
ryy_test_cases_2 = generate_ryy_test_cases(nr_rng_trials=N_RNG_TRIALS, use_native=True)
